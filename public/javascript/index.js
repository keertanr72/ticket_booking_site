const createUser = async (event) => {
    ...
    try {
        const check = await axios.post(('http://http://3.95.218.68:3000/user/sign-up/check-email'), { email })
    }
    catch (error) {
    ...
    try {
        const data = await axios.post('http://http://3.95.218.68:3000/user/sign-up', userDetails)
        if (data.status === 201) {
            alert('success')
