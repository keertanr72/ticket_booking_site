...
document.addEventListener('DOMContentLoaded', function() {
    ...
    orderButton.addEventListener('click', async () => {
        try {
            ...
            const membershipData = await axios.post('http://localhost:3000/booking', { data }, { headers: { 'Authorization': token } })
            console.log(membershipData)
            const options = {
                'key': membershipData.data.key_id,
                'order_id': membershipData.data.order.id,
                'handler': async (membershipData) => {
                    const orderDetails = await axios.post('http://localhost:3000/booking/payment-success', {
            ...
            rzp1.on('payment.failed', async () => {
                await axios.put('http://localhost:3000/booking/payment-fail', { data }, { headers: { 'Authorization': token } })
