...
async function handleFormSubmit() {
    ...
    try {
        await Promise.all(selectedSeats.map(async (ele) => {
            let temp = await axios.get(`http://3.95.218.68:3000/seats/${ele.seatNumber}?class=${ele.seat_class}`);
            data.push(temp.data);
        }));
        data.forEach(ele => {
            console.log(ele);
...
const loadingContents = async () => {
    const seatData = await axios.get('http://3.95.218.68:3000/seats');
...
document.getElementById('previousOrdersBtn').addEventListener('click', async () => {
    let userData = await axios.get('http://3.95.218.68:3000/booking', { headers: { 'Authorization': token } })
